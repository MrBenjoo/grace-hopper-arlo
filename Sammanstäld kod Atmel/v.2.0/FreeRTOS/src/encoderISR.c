...
/////////////////////////PULSE TIMES///////////////////////////////////////
uint8_t getRightAPulseTime(){ //returns the latest pulse time for the right encoder A in milliseconds
	printf("RightPulseTime A: %i ms\n",rAPulseTime);
	return rAPulseTime;
	
}
uint8_t getRightBPulseTime(){ //returns the latest pulse time for the right encoder B  in milliseconds
	printf("RightPulseTime B: %i ms\n",rBPulseTime);
	return rBPulseTime;
}
uint8_t getLeftAPulseTime(){  //returns the latest pulse time for the left encoder A in milliseconds
	printf("LeftPulseTime A: %i ms\n",lAPulseTime);
	return lAPulseTime;
}
uint8_t getLeftBPulseTime(){  //returns the latest pulse time for the left encoder B in milliseconds
	printf("LeftPulseTime B: %i ms\n",lBPulseTime);
	return lBPulseTime;
}
