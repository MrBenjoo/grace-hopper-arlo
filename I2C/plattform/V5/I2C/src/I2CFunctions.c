...
uint8_t send_data_pab[TWI_DATA_SEND_LENGTH_PA]	= {};	/* stores the data which will be sent to påbyggnad */
uint8_t send_data_pos[TWI_DATA_SEND_LENGTH_POS] = {};	/* stores the data which will be sent to positionering */
uint8_t rec_data_pab[TWI_DATA_REC_LENGTH_PA]	= {};	/* stores the data which will be received from påbyggnad */
...
uint8_t send_package(uint8_t data, uint8_t slave)
{
	...
	if(TWI_SLAVE_ADR_PAB == slave)
	{
		...
		/* Används som test för att skicka 3 bytes, dock måste TWI_DATA_SEND_LENGTH_PA ändras till 3 */
	...
	else if(TWI_SLAVE_ADR_POS == slave)
	{
		...
		/* Används som test för att skicka 3 bytes, dock måste TWI_DATA_SEND_LENGTH_POS ändras till 3 */
