...
void taskPAB(void *pvParamters)
{
	...
	uint8_t current_state = SKICKA_ID;
	semaphorePAB = 0;
	...
	while (1)
	{
		...
		if(xSemaphoreTake(semaphorePAB, 0) == pdTRUE)
		{
			...
			{
				case SKICKA_ID:
				if(send_package(SKICKA_ID, TWI_SLAVE_ADR_PAB) == DATA_SENT)
				{
					printf("->sending SKICKA_ID to PAB\n");
				...
				if(read_package(TWI_SLAVE_ADR_PAB) == DATA_READ)
				{
					switch(rec_data_pab[0])
					{
						...
						case VINGLASS_ID:
						printf("received VINGLASS_ID\n");
						printf("%x\n",rec_data_pab[0]);
						current_state = BORJA_PLOCKA;
						break;
						
						case KUB_ID:
						printf("received KUB_ID\n");
						printf("%x\n",rec_data_pab[0]);
						current_state = BORJA_PLOCKA;
						break;
					}
